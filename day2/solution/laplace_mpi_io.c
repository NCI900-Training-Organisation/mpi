...
while (iter< max_iter)
{
    ...
    MPI_Recv(submesh[*ptr_rows -1], mesh_size, MPI_DOUBLE, higher, highertag, world, MPI_STATUS_IGNORE);
    MPI_Send(submesh[1], mesh_size, MPI_DOUBLE, lower, highertag, world);


    /* communicate to the lower rank process */
    MPI_Recv(submesh[0], mesh_size, MPI_DOUBLE, lower, lowertag, world, MPI_STATUS_IGNORE);
