...
#TODO: use one window object to host both upper and lower bnd data.
...
while (iter<max_iter)
{
    ...
    MPI_Put(submesh[*ptr_rows - 2], mesh_size, MPI_DOUBLE, higher, 0, mesh_size, MPI_DOUBLE, lower_win);   
    MPI_Win_fence(0, lower_win);
...
MPI_Free_mem(upper_bnd);
MPI_Free_mem(lower_bnd);
